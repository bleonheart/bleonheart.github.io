(() => {
  ...
  function create(application) {
    let result;
    if (application.game === "tetris" || application.id === "tetris") result = createTetris();
    else if (application.game === "pong" || application.id === "pong") result = createPong();
    else {
      const shell = makeShell(application.label || "Game", "This game is not available.");
      shell.status.textContent = "Unavailable";
      result = { element: shell.element, controller: null };
    }
    if (result?.element) gameAudio.bind(result.element);
    return result;
  }

  window.PortfolioGames = { create };
})();
